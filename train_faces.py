...
for lab in labs:
    if (lab == my_faces_path):
        labs.append(np.array([0, 1]))
    else:
        labs.append(np.array([1, 0]))
"""
...
x = tf.placeholder(tf.float32, [None, size, size, 3])  # 第二个参数是shape
y_ = tf.placeholder(tf.float32, [None, 2])
# dropout rates, 0.5 and 0.75
keep_prob_5 = tf.placeholder(tf.float32)
keep_prob_75 = tf.placeholder(tf.float32)


def weightVariable(shape):
    # 以shape为维度生成矩阵，元素是均值是0，标准差为0.01的随机数（正态分布）
    init = tf.random_normal(shape, stddev=0.01)
    return tf.Variable(init)


def biasVariable(shape):
    # 通常来说偏执项会使用常数，这里不知道为什么作者这样写
...
def cnnTrain():
    ...
    with tf.Session() as sess:
        ...
        summary_writer = tf.summary.FileWriter(
            './tmp', graph=tf.get_default_graph())
            ...
            for i in range(num_batch):
                ...
                if (n*num_batch+i) % 100 == 0:
                    ...
                    if acc > 0.98 and n > 2:
                        ...
                        sys.exit(0)
                        print("training done with accuray at:%f"%acc)
